# Design Document: Do Models Quietly Downgrade Users They Judge Immoral

## 1. Purpose

This document is the method contract for **Do Models Quietly Downgrade Users They Judge Immoral**. It specifies what is
...
One-liner: Measure whether models degrade HHH quality when they infer a user has poor moral character.

Hypothesis: Measure whether models degrade HHH quality when they infer a user has poor moral character.
...
|---|---|---|
| Config schema | `moralban.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `moralban.configs.loader` | Hydra compose API |
| Device | `moralban.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `moralban.models.registry` | Pinned revisions + hardware fit |
| Hooks | `moralban.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `moralban.models.generation` | Chat template path recording |
| Data | `moralban.data` | Synthetic items, splits, manifests |
| Evaluation | `moralban.evaluation` | Metrics, failure harness, protocols |
| Ablation | `moralban.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `moralban.reporting` | Aggregate → MD/TeX/figures |
| Cache | `moralban.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/moral-shadowban
...
`src/moralban/moralban/` are preserved across regeneration.
